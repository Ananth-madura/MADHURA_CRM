...
if errorlevel 1 (
  ...
  if errorlevel 1 (
    ...
    echo Please accept the UAC prompt to allow installation of ACHME CRM.
    echo ====================================================================
    echo.
    pause
    exit /b 1
...
:: Free port if occupied (e.g. by boot task running as SYSTEM)
...
:: ====================================================================
echo  [9/%S%] Configuring auto-boot, firewall, hosts, and power settings...
echo [%DATE% %TIME%] Step 9: Auto-boot, firewall, hosts, and power settings >> "%LOGFILE%"
...
:: --- Run Auto-Boot, Hosts, and Power Config installer silently ---
if exist "%ROOT%\install-boot-startup.bat" (
  echo         Installing boot startup tasks, power config, and local hosts...
  call "%ROOT%\install-boot-startup.bat" /silent
  if not errorlevel 1 (
    echo         Automatic Boot Startup installer completed successfully. [OK]
  ) else (
    echo         [WARN] Boot Startup installer finished with errors.
  )
) else (
  echo         [WARN] install-boot-startup.bat not found - skipping automatic boot tasks.
)
