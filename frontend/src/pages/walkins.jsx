...
import { Search, Plus, X, ChevronDown, Trash2, Edit, FileText, History, Bell, Clock, MessageCircle } from "lucide-react";
...
const Walkins = () => {
  ...
  const fetchWalkins = async () => {
    try {
      ...
      const res = await axios.get(`${API}/api/Walkins`, config);
      setWalkins(res.data);
  ...
  const fetchMissedCounts = async () => {
    try {
      const res = await axios.get(`${API}/api/leads/missed-counts/walkin`, getAuthConfig());
      const map = {};
  ...
  useEffect(() => {
    fetchWalkins(); fetchMissedCounts();
  ...
  const openHistory = async (lead) => {
    ...
    try {
      const [actRes, remRes, folRes] = await Promise.all([
        axios.get(`${API}/api/leads/activity/walkin/${lead.id}`, getAuthConfig()),
        axios.get(`${API}/api/leads/reminders/walkin/${lead.id}`, getAuthConfig()),
        axios.get(`${API}/api/leads/followups/walkin/${lead.id}`, getAuthConfig()),
      ]);
  ...
  const openReminderPanel = async (lead) => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/api/leads/reminders/walkin/${lead.id}`, getAuthConfig());
  ...
  const openFollowupPanel = async (lead) => {
    setFollowupLeadId(lead.id); setFollowupLeadName(lead.customer_name);
    setNewFollowupDate(""); setNewFollowupTime(""); setNewFollowupNote("");
    try {
      const res = await axios.get(`${API}/api/leads/followups/walkin/${lead.id}`, getAuthConfig());
  ...
  const saveFollowup = async () => {
    ...
    try {
      await axios.post(`${API}/api/leads/followups`, {
        lead_id: followupLeadId, lead_type: "walkin",
        followup_date: newFollowupDate, followup_time: newFollowupTime || null, followup_notes: newFollowupNote,
      }, getAuthConfig());
      await axios.patch(`${API}/api/Walkins/${followupLeadId}`, { followup_required: "Yes", followup_date: newFollowupDate }, getAuthConfig()).catch(() => { });
      const res = await axios.get(`${API}/api/leads/followups/walkin/${followupLeadId}`, getAuthConfig());
      setLeadFollowups(res.data); fetchWalkins();
      setNewFollowupDate(""); setNewFollowupTime(""); setNewFollowupNote("");
  ...
  const saveWalkins = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`${API}/api/Walkins/${editId}`, payload, getAuthConfig());
      ...
      setOpen(false);
      setIsEdit(false);
      if (["Converted", "Closed", "Billed"].includes(payload.walkin_status)) {
  ...
  const openEdit = async (id) => {
    try {
      const res = await axios.get(`${API}/api/Walkins/${id}`, getAuthConfig());
      const data = res.data;
  ...
  const deletefield = async (id) => {
    ...
    try {
      await axios.delete(`${API}/api/Walkins/${id}`, getAuthConfig());
      fetchWalkins();
    } catch (err) {
  ...
  const uniqueCreators = Array.from(new Set(Walkings.map(item => item.creator_name).filter(Boolean)));

  const filteredWalkins = Walkings.filter(T => {
    ...
    if (outcomeFilter && outcomeFilter !== "All") {
      matchesOutcome = T.walkin_status === outcomeFilter;
    }
    ...
    <div className="w-full">
      <div className="invoice-heading-tab flex gap-4 justify-between item-center">
        ...
        <div className="flex gap-3 ">
          ...
          <div className="mt-2">
            <button
              onClick={() => { setIsEdit(false); setForm({ customer_name: "", company_name: "", mobile_number: "", location_city: "", walkin_date: getToday(), purpose: "", staff_name: "", walkin_status: "New", followup_required: "Default", followup_date: getToday(), followup_notes: "", reminder_required: "Default", reminder_date: getToday(), reminder_time: "", reminder_notes: "", reference: "", email: "", gst_number: "", landline_number: "", alternate_mobile_number: "", reference_by: "", nearest_landmark: "" }); setCustomOutcomeInput(""); setOpen(true); }}
      ...
      <div className="application-maintab  p-5 ">
        <div className={`overlay ${open ? "show" : ""} flex justify-center items-start overflow-y-auto p-4 md:p-10 z-50`}>
          <div className={`task-application bg-white shadow w-full max-w-4xl overflow-y-auto p-6 md:p-8 rounded-xl my-8 z-50  ${open ? "show" : ""
            }`}>
            {/*  */}
            ...
            <form action="" onSubmit={saveWalkins} className=" invoice-form p-6 space-y-6 relative">
              <div className="grid grid-cols-4 items-center gap-6">
                <label htmlFor="" className="text-sm text-gray-600 text-left">Customer Name<span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                <label htmlFor="" className="text-sm text-gray-600 text-left">Company Name</label>
                <input type="text" value={form.company_name} onChange={handleChange} name="company_name" placeholder="e.g. Acme Corp" className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                <label className="text-sm text-gray-600 text-left">Landline Number</label>
                <input type="text" value={form.landline_number || ""} onChange={handleChange} name="landline_number" className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>

              <div className="grid grid-cols-4 items-center gap-6">
                <label className="text-sm text-gray-600 text-left">Alternate Mobile Number</label>
                <input type="text" value={form.alternate_mobile_number || ""} onChange={handleChange} name="alternate_mobile_number" maxLength={13} inputMode="numeric" onKeyDown={e => { if (!/[0-9]/.test(e.key) && !["Backspace", "Delete", "ArrowLeft", "ArrowRight", "Tab"].includes(e.key)) e.preventDefault(); }} className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                <label className="text-sm text-gray-600 text-left">Nearest Landmark</label>
                <input type="text" value={form.nearest_landmark || ""} onChange={handleChange} name="nearest_landmark" className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                <label className="text-sm text-gray-600 text-left">Reference By</label>
                <input type="text" value={form.reference_by || ""} onChange={handleChange} name="reference_by" className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>
                    ...
                    <div className="border border-yellow-200 rounded-xl p-4 bg-yellow-50/40">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          {form.reminder_time && <p className="text-xs text-yellow-700 font-semibold mt-1 flex items-center gap-1">🔔 Reminder set for <span className="bg-yellow-100 px-1.5 py-0.5 rounded font-bold">{to12h(form.reminder_time)}</span> — alerts 10 min &amp; 5 min before</p>}
                        </div>
              ...
              <div className="flex flex-wrap gap-3 pt-4 more2">
                ...
                <button type="button" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("qt_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "walkin" }));
                    sessionStorage.setItem("qt_prefill_tab", "quotation");
                    window.location.href = "/dashboard/proposal";
                  }}>
                  <FileText size={15} /> Create Quotation
                </button>
                <button type="button" className="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("qt_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "walkin" }));
                ...
                <button type="button" className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("qt_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "walkin" }));
                ...
                <button type="button" className="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("contract_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "walkin" }));
          ...
          <table className="w-full min-w-[900px] text-sm border border-gray-300">
            ...
            <tbody className="text-sm font-[Times-New-Roman] text-center">
              {filteredWalkins.map((W) => {
                ...
                return (
                  <tr key={W.id} className="hover:bg-gray-100 hover:text-black transition">
                    ...
                    <td className="border px-4 py-2 text-left font-medium">{W.company_name || "---"}</td>
                    <td className="border px-4 py-2 whitespace-nowrap">{W.mobile_number}</td>
                    <td className="border px-4 py-2">{W.reference_by || "---"}</td>
                    ...
                    <td className="border px-4 py-2 text-center">
                      <div className="flex justify-center gap-2 items-center">
                        <a
                          href={`https://wa.me/91${(W.mobile_number || "").replace(/\D/g, "").slice(-10)}`}
                          target="_blank"
                          rel="noreferrer"
                        ...
                        <button
                          type="button"
                          onClick={() => openFollowupPanel(W)}
                          title="Follow-ups"
                          className={`p-1.5 rounded-lg transition-all ${W.followup_required === "Yes"
              ...
              {filteredWalkins.length === 0 && (
                <tr>
                  <td colSpan={(isAdmin || user?.role === "subadmin") ? 9 : 8} className="py-10 text-gray-400 italic text-center">
                    No walkin records found
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex justify-center items-start overflow-y-auto pt-10 pb-10">
          <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm bg-gray-50 rounded-xl p-4 mb-5 border">
              ...
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Company Name</span><div className="mt-0.5 font-medium">{historyLead.company_name || "---"}</div></div>
              ...
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Landline No</span><div className="mt-0.5">{historyLead.landline_number || "---"}</div></div>
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Alt Mobile No</span><div className="mt-0.5">{historyLead.alternate_mobile_number || "---"}</div></div>
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Landmark</span><div className="mt-0.5">{historyLead.nearest_landmark || "---"}</div></div>
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Reference By</span><div className="mt-0.5">{historyLead.reference_by || "---"}</div></div>
            </div>
            <div className="mb-5">
              ...
              {historyReminders.length === 0 ? <p className="text-xs text-gray-400 italic bg-gray-50 rounded-lg p-3">No reminders set.</p> : (
                <div className="space-y-2">{historyReminders.map((r, idx) => (
                  <div key={r.id} className={`flex items-center gap-3 p-3 rounded-lg border text-sm ${r.status === "Missed" ? "bg-red-50 border-red-200" : r.status === "Done" ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-200"}`}>
                    ...
                    <div className="flex-1"><div className="font-semibold text-gray-800">{formatReminderDate(r.reminder_date)}{r.reminder_time ? ` · ${to12h(r.reminder_time)}` : ""}</div>{r.reminder_notes && <div className="text-gray-500 text-xs mt-0.5">{r.reminder_notes}</div>}</div>
            ...
            <div className="mb-5">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-2"><Clock size={13} className="text-blue-500" /> Follow-up Log <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">{historyFollowups.length}</span></h3>
              {historyFollowups.length === 0 ? <p className="text-xs text-gray-400 italic bg-gray-50 rounded-lg p-3">No follow-ups set.</p> : (
                <div className="space-y-2">{historyFollowups.map((f, idx) => (
                  <div key={f.id} className={`flex items-center gap-3 p-3 rounded-lg border text-sm ${f.status === "Done" ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-200"}`}>
                    <div className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white bg-blue-400">{idx + 1}</div>
                    <div className="flex-1"><div className="font-semibold text-gray-800">{formatReminderDate(f.followup_date)}{f.followup_time ? ` · ${to12h(f.followup_time)}` : ""}</div>{f.followup_notes && <div className="text-gray-500 text-xs mt-0.5">{f.followup_notes}</div>}</div>
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${f.status === "Done" ? "bg-green-600 text-white" : "bg-blue-600 text-white"}`}>{f.status}</span>
                  </div>
                ))}</div>
              )}
            </div>
                ...
                <div>
                  ...
                  {newReminderTime && (
                    <p className="text-xs text-yellow-700 font-semibold mt-1">
                      🔔 <span className="bg-yellow-100 px-1.5 py-0.5 rounded font-bold">{to12h(newReminderTime)}</span>
                    </p>
                  )}
                </div>
              </div>
            ...
            {leadReminders.length === 0 ? <p className="text-xs text-gray-400 italic">No reminders yet.</p> : (
              <div className="space-y-2">{leadReminders.map(r => (
                <div key={r.id} className={`flex items-center justify-between p-3 rounded-lg border text-sm ${r.status === "Missed" ? "bg-red-50 border-red-200" : r.status === "Done" ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-200"}`}>
                  <div><div className="font-semibold">{formatReminderDate(r.reminder_date)}{r.reminder_time ? ` at ${to12h(r.reminder_time)}` : ""}</div>{r.reminder_notes && <div className="text-gray-500 text-xs mt-0.5">{r.reminder_notes}</div>}</div>
      ...
      {followupOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 flex justify-center items-start overflow-y-auto pt-10 pb-10">
          <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-lg p-6">
            <div className="flex justify-between items-center mb-4 border-b pb-3">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Clock size={18} className="text-blue-500" /> Follow-ups — {followupLeadName}</h2>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs text-gray-500 font-semibold">Time</label>
                  <input type="time" value={newFollowupTime} onChange={e => setNewFollowupTime(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm outline-none mt-1" />
                  {newFollowupTime && (
                    <p className="text-xs text-blue-700 font-semibold mt-1">
                      📞 <span className="bg-blue-100 px-1.5 py-0.5 rounded font-bold">{to12h(newFollowupTime)}</span>
                    </p>
                  )}
                </div>
              </div>
              <div><label className="text-xs text-gray-500 font-semibold">Reason / Note</label><input type="text" value={newFollowupNote} onChange={e => setNewFollowupNote(e.target.value)} placeholder="e.g. Send revised quotation" className="w-full border rounded-lg px-3 py-2 text-sm outline-none mt-1" /></div>
              <button onClick={saveFollowup} className="w-full bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-blue-700 text-sm">+ Add Follow-up</button>
            </div>
            ...
            {leadFollowups.length === 0 ? <p className="text-xs text-gray-400 italic">No follow-ups yet.</p> : (
              <div className="space-y-2">{leadFollowups.map(f => (
                <div key={f.id} className={`flex items-center justify-between p-3 rounded-lg border text-sm ${f.status === "Done" ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-200"}`}>
                  <div><div className="font-semibold">{formatReminderDate(f.followup_date)}{f.followup_time ? ` at ${to12h(f.followup_time)}` : ""}</div>{f.followup_notes && <div className="text-gray-500 text-xs mt-0.5">{f.followup_notes}</div>}</div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${f.status === "Done" ? "bg-green-600 text-white" : "bg-blue-600 text-white"}`}>{f.status}</span>
