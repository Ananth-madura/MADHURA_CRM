...
const Fields = () => {
  ...
  const fetchFields = async () => {
    try {
      ...
      const res = await axios.get(`${API}/api/fields`, config);
      setFields(res.data);
  ...
  const openHistory = async (lead) => {
    ...
    try {
      const [actRes, remRes, folRes] = await Promise.all([
        ...
        axios.get(`${API}/api/leads/followups/field/${lead.id}`, getAuthConfig()),
      ]);
  ...
  const openFollowupPanel = async (lead) => {
    ...
    try {
      const res = await axios.get(`${API}/api/leads/followups/field/${lead.id}`, getAuthConfig());
  ...
  const saveFollowup = async () => {
    ...
    try {
      await axios.post(`${API}/api/leads/followups`, {
        lead_id: followupLeadId, lead_type: "field",
        followup_date: newFollowupDate, followup_time: newFollowupTime || null, followup_notes: newFollowupNote,
      }, getAuthConfig());
      await axios.patch(`${API}/api/fields/${followupLeadId}`, { followup_required: "Yes", followup_date: newFollowupDate }, getAuthConfig()).catch(() => { });
      const res = await axios.get(`${API}/api/leads/followups/field/${followupLeadId}`, getAuthConfig());
      setLeadFollowups(res.data); fetchFields();
      setNewFollowupDate(""); setNewFollowupTime(""); setNewFollowupNote("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setOpen(false);
      setIsEdit(false);
      if (["Converted", "Closed", "Billed"].includes(payload.field_outcome)) {
  ...
  const uniqueCreators = Array.from(new Set(fields.map(item => item.creator_name).filter(Boolean)));

  const filteredFields = fields.filter(T => {
    ...
    if (dateTypeFilter === "lead_date") {
      targetDateStr = T.visit_date || T.call_date || T.walkin_date;
    ...
    if (outcomeFilter && outcomeFilter !== "All") {
      matchesOutcome = T.field_outcome === outcomeFilter;
    }
    ...
    <div className="w-full">
      <div className="invoice-heading-tab flex gap-4 justify-between item-center">
        ...
        <div className="flex gap-3 ">
          ...
          <div className="mt-2">
            <button
              onClick={() => { setIsEdit(false); setForm({ customer_name: "", company_name: "", mobile_number: "", location_city: "", visit_date: getToday(), purpose: "", staff_name: "", field_outcome: "New", followup_required: "Default", followup_date: getToday(), followup_notes: "", reminder_required: "Default", reminder_date: getToday(), reminder_notes: "", reference: "", email: "", gst_number: "", landline_number: "", alternate_mobile_number: "", reference_by: "", nearest_landmark: "" }); setCustomOutcomeInput(""); setOpen(true); }}
          ...
          <div className={`task-application bg-white shadow w-full max-w-4xl overflow-y-auto p-6 md:p-8 rounded-xl my-8 z-50  ${open ? "show" : ""
            ...
            <form onSubmit={handleSubmit} action="" className=" invoice-form p-6 space-y-6 relative">
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                ...
                <input type="text" name="company_name" value={form.company_name} onChange={handleChange} placeholder="e.g. Acme Corp" className="col-span-3 border rounded-md px-3 py-2 outline-none bg-white w-[100%]" />
              </div>
              ...
              {/* submit and close */}
              <div className="flex flex-wrap gap-3 pt-4 z-30 more">
                ...
                <button type="button" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("qt_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "field" }));
                ...
                <button type="button" className="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 flex items-center gap-2 text-sm"
                  onClick={() => {
                    sessionStorage.setItem("contract_prefill", JSON.stringify({ customer_name: form.customer_name, company_name: form.company_name, mobile_number: form.mobile_number, email: form.email || "", location_city: form.location_city, lead_id: editId || null, lead_type: "field" }));
      ...
      <div className="bg-white shadow rounded-xl overflow-x-auto mt-5">
        <table className="w-full min-w-[900px] text-sm border border-gray-200 border-collapse">
          <thead className="bg-[#f8faf9] rounded-xl">
            <tr className="text-black font-[Times-New-Roman] border-b border-gray-300 uppercase text-xs">
              ...
              <th className="px-4 py-3 border">Reference By</th>
            ...
            {filteredFields.map((f) => {
              ...
              return (
                <tr key={f.id} className="border-b border-gray-200 hover:bg-gray-50 transition cursor-pointer">
                  ...
                  <td className="px-4 py-3 border text-left font-medium">{f.company_name || "---"}</td>
                  <td className="px-4 py-3 border">{f.mobile_number}</td>
                  <td className="px-4 py-3 border">{f.reference_by || "---"}</td>
                  ...
                  <td className="px-4 py-3 border">
                    <div className="flex gap-2 justify-center items-center">
                      <a
                        href={`https://wa.me/91${(f.mobile_number || "").replace(/\D/g, "").slice(-10)}`}
                        target="_blank"
                        rel="noreferrer"
                      ...
                      <button
                        type="button"
                        onClick={() => openFollowupPanel(f)}
                        title="Follow-ups"
                        className={`p-1.5 rounded-lg transition-all ${f.followup_required === "Yes"
          ...
          <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-lg p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs text-gray-500 font-semibold">Time</label>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex justify-center items-start overflow-y-auto pt-10 pb-10">
          <div className="bg-white rounded-xl shadow-2xl w-[95%] max-w-lg p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-3">
              ...
              <div><label className="text-xs text-gray-500 font-semibold">Reason / Note</label><input type="text" value={newFollowupNote} onChange={e => setNewFollowupNote(e.target.value)} placeholder="e.g. Revisit site next week" className="w-full border rounded-lg px-3 py-2 text-sm outline-none mt-1" /></div>
