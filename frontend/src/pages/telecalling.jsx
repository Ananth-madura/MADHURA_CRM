...
const Telecall = () => {
  ...
  const OUTCOME_OPTIONS = ["New", "Hot Case", "Warm Case", "Cold Case", "Not Required", "Converted", "Closed", "Billed", "Custom"];
  const tabopen = () => {
    setOpen(true);
  };
  ...
  const openEdit = async (id) => {
    try {
      ...
      setForm({
        ...
        reminder_date: data.reminder_date ? data.reminder_date.toString().slice(0, 10) : "",
    ...
    <div className="w-full">
      <div className="invoice-heading-tab flex gap-4 justify-between item-center">
        ...
        <div className="flex gap-3">
          ...
          <div className="mt-2">
            <button
              onClick={() => { setIsEdit(false); setForm({ customer_name: "", mobile_number: "", location_city: "", call_date: new Date().toISOString().slice(0, 10), service_name: "", staff_name: "", call_outcome: "New", followup_required: "Default", followup_date: new Date().toISOString().slice(0, 10), followup_notes: "", reminder_required: "Default", reminder_date: new Date().toISOString().slice(0, 10), reminder_time: "", reminder_notes: "", reference: "", gst_number: "", email: "" }); setCustomOutcomeInput(""); setOpen(true); }}
      ...
      <div className="application-maintab  p-5">
        <div className={`overlay ${open ? "show" : ""} justify-items-center pb-[50px]`}>
          <div className={`task-application bg-white shadow ml-[50px] mt-[65px] w-[65%] z-70  p-10 rounded-lg  ${open ? "show" : ""
            }`}>
            {/*  */}
            ...
            <form onSubmit={saveTelecall} className=" invoice-form p-6 space-y-6 relative">
              ...
              {/* Description */}
              <div className="grid grid-cols-4 items-center gap-6">
                <label htmlFor="" className="text-sm text-gray-600 text-left">Purpose of call</label>
              ...
              <div className="grid grid-cols-4 items-center gap-6">
                ...
                <div className="relative col-span-3">
                  ...
                  {form.call_outcome === "Custom" && (
                    <div className="mt-2">
                      <input type="text" value={customOutcomeInput} onChange={e => { setCustomOutcomeInput(e.target.value); setForm({ ...form, call_outcome: e.target.value }); }} placeholder="Enter custom outcome..." className="border rounded-md px-3 py-2 outline-none w-full bg-white text-sm" autoFocus />
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <div className="border border-yellow-200 rounded-xl p-4 bg-yellow-50/40">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <input type="time" value={form.reminder_time} onChange={handleChange} name="reminder_time"
                            className="border rounded-lg px-3 py-2 outline-none bg-white w-full text-sm mt-1" />
                          {form.reminder_time && (
                            <p className="text-xs text-yellow-600 mt-1">You'll be notified at {form.reminder_time.split(':').map((p,i) => i===0 ? String(Math.max(0,parseInt(p)-0)).padStart(2,'0') : p).join(':')} — 10 min before & 5 min before</p>
                          )}
                        </div>
                        <div>
                          <label className="text-xs font-semibold text-gray-500">Notes</label>
                  ...
                  <tr key={T.id} className="hover:bg-gray-100 hover:text-black transition">
                    ...
                    <td className="border px-4 py-2">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-xs font-semibold text-blue-600">{T.created_by === user?.id ? "Me" : (T.creator_name || "Admin")}</span>
                      </div>
                    </td>
                    ...
                    <td className="border px-4 py-2 text-center">
                      <div className="flex justify-center gap-2 items-center">
                        ...
                        <button type="button" onClick={async () => {
                          try {
                            await axios.patch(`${API}/api/Telecalls/${T.id}`, { followup_required: "Yes", followup_date: new Date().toISOString().slice(0, 10) }, getAuthConfig());
                            fetchTelecalls();
                        ...
                        <button type="button" onClick={() => openEdit(T.id)} title="Edit" className="text-green-600 hover:text-green-800 p-1.5 rounded-lg hover:bg-green-50 transition-all"><Edit size={16} /></button>
                        {isAdmin && <button type="button" onClick={() => deletefield(T.id)} title="Delete" className="text-red-500 hover:text-red-700 p-1.5 rounded-lg hover:bg-red-50 transition-all"><Trash2 size={16} /></button>}
                      </div>
                    </td>
                  </tr>
                );
            ...
            <div className="grid grid-cols-2 gap-2 text-sm bg-gray-50 rounded-xl p-4 mb-5 border">
              <div><span className="text-gray-400 text-xs font-semibold uppercase">Status</span>
                <div className="mt-0.5">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${historyLead.call_outcome === "Hot Case" ? "bg-red-100 text-red-700" :
              ...
              {historyReminders.length === 0 ? (
                ...
                <div className="space-y-2">
                  {historyReminders.map((r, idx) => (
                    <div key={r.id} className={`flex items-center gap-3 p-3 rounded-lg border text-sm ${r.status === "Missed" ? "bg-red-50 border-red-200" :
                      ...
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${r.status === "Missed" ? "bg-red-600 text-white" :
