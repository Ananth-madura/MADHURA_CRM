...
function fmtNum(n, showSym = true) {
  const v = Number(n || 0);
...
const BRANCHES = {
  Coimbatore: { name: "Coimbatore", address: "Opp to SMS Hotel, Peelamedu, Avinashi Road, Coimbatore-641004", gstin: "33AAHFA7876M1ZX", phone: "0422 4397555 , 2563666" },
  Bangalore: { name: "Bangalore", address: "14th Main Road, GK Layout, Electronic City Post, Bangalore-560100", gstin: "29AAHFA7876M1ZM", phone: "9842235515, 8012555718" },
  Chennai: { name: "Chennai", address: "5th Floor, 5CD PM Towers, Greams Road, Thousand Lights, Chennai-600006", gstin: "33AAHFA7876M1ZX", phone: "8012555706, 8012555710" },
};
...
function generateEmailHtml({ invoice, items, type, docNumber, docLabel, companyName, companyEmail, companyPhone, companyWebsite, companyGstin, companyAddress }) {
  ...
  const itemRows = (items || []).map((item, i) => {
    ...
    return `<tr>
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;font-size:11px;color:#1a1f2e;vertical-align:top;font-family:Arial,sans-serif;max-width:160px;word-break:break-word;">
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;font-size:11px;color:#444;vertical-align:top;text-align:right;font-family:Arial,sans-serif;">${fmtNum(price, false)}</td>
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #e2e8f0;font-size:11px;color:#1a1f2e;vertical-align:top;text-align:right;font-family:Arial,sans-serif;font-weight:bold;">${fmtNum(lineTotal, false)}</td>
    </tr>`;
  }).join("");

  const emailGstMode = h.gst_mode || "Exclusive";
  const emailGrossBase = subtotal - totalDiscount;
  const emailTaxBase = emailGstMode === "Inclusive"
    ? Math.max(emailGrossBase - totalCGST - totalSGST - totalIGST, 0)
    : emailGrossBase;
  const emailCgstPct = emailTaxBase > 0 && totalCGST > 0 ? +((totalCGST / emailTaxBase) * 100).toFixed(2).replace(/\.?0+$/, '') : 0;
  const emailSgstPct = emailTaxBase > 0 && totalSGST > 0 ? +((totalSGST / emailTaxBase) * 100).toFixed(2).replace(/\.?0+$/, '') : 0;
  const emailIgstPct = emailTaxBase > 0 && totalIGST > 0 ? +((totalIGST / emailTaxBase) * 100).toFixed(2).replace(/\.?0+$/, '') : 0;
  ...
  const summaryRows = `
    ...
    ${totalCGST > 0 ? `<tr><td style="padding:10px 8px;font-size:12px;color:#64748b;width:50%;font-family:Arial,sans-serif;">CGST${emailCgstPct > 0 ? ` (${emailCgstPct}%)` : ""}</td><td style="padding:10px 8px;font-size:12px;color:#1a1f2e;text-align:right;font-weight:700;width:50%;font-family:Arial,sans-serif;">${fmtNum(totalCGST)}</td></tr>` : ""}
    ${totalSGST > 0 ? `<tr><td style="padding:10px 8px;font-size:12px;color:#64748b;width:50%;font-family:Arial,sans-serif;">SGST${emailSgstPct > 0 ? ` (${emailSgstPct}%)` : ""}</td><td style="padding:10px 8px;font-size:12px;color:#1a1f2e;text-align:right;font-weight:700;width:50%;font-family:Arial,sans-serif;">${fmtNum(totalSGST)}</td></tr>` : ""}
    ${totalIGST > 0 ? `<tr><td style="padding:10px 8px;font-size:12px;color:#64748b;width:50%;font-family:Arial,sans-serif;">IGST${emailIgstPct > 0 ? ` (${emailIgstPct}%)` : ""}</td><td style="padding:10px 8px;font-size:12px;color:#1a1f2e;text-align:right;font-weight:700;width:50%;font-family:Arial,sans-serif;">${fmtNum(totalIGST)}</td></tr>` : ""}
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f1f5f9;padding:20px 10px;">
    <tr>
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                  <td valign="top" style="text-align:left;font-family:Arial,sans-serif;">
                    ...
                    <div style="color:#1a1f2e;font-size:12px;margin-top:4px;font-family:Arial,sans-serif;">Ph: ${(() => { const raw = companyPhone || (BRANCHES[h.supplier_branch] && BRANCHES[h.supplier_branch].phone) || "0422 4397555 , 2563666"; return esc(raw.split(",").map(p => `+91 ${p.trim()}`).join("  |  ")); })()} | Email: ${esc(companyEmail || "sales@achmecommunication.com")}</div>
                  </td>
      ...
      return `<div style="font-size:15px;font-weight:bold;color:#2c2c2c;margin-bottom:4px;">${esc(clientCompany || h.customer_name || "---")}</div>
              ...
              ${h.mobile_number ? `<div style="font-size:12px;color:#1a1f2e;margin-top:4px;">Ph: +91 ${esc(h.mobile_number)}</div>` : ""}
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
                <thead>
                  <tr style="background-color:#f8fafc;">
                    ...
                    <th style="padding:10px 8px;border-bottom:2px solid #1e3a8a;color:#1e293b;font-size:11px;font-weight:bold;text-align:left;font-family:Arial,sans-serif;width:160px;max-width:160px;">DESCRIPTION</th>
                    <th style="padding:10px 8px;border-bottom:2px solid #1e3a8a;color:#1e293b;font-size:11px;font-weight:bold;text-align:left;font-family:Arial,sans-serif;">BRAND</th>
                    ...
                    <th style="padding:10px 8px;border-bottom:2px solid #1e3a8a;color:#1e293b;font-size:11px;font-weight:bold;text-align:right;font-family:Arial,sans-serif;">UNIT PRICE</th>
                    ...
                    <th style="padding:10px 8px;border-bottom:2px solid #1e3a8a;color:#1e293b;font-size:11px;font-weight:bold;text-align:right;font-family:Arial,sans-serif;">TOTAL VALUE</th>
                  </tr>
                </thead>
                <tbody>
                  ${itemRows}
